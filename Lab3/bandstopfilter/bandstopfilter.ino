...
int lpFilteredSample = 127;   // lågpass nedre
int lpFilteredSample2 = 127;  // lågpass övre
int bpFilteredSample = 127;   // bandpass
int bsFilteredSample = 127;   // bandstopp (notch)
...
void setup()
{
  ...
  lpFilteredSample2 = 127;
  bpFilteredSample = 127;
  bsFilteredSample = 127;
...
void loop()
{
  ...
  //  Lågpass alpha,begränsning 4%-78% 
  ...
  float lpAlpha2 = sqrt(lpAlpha);
  if (lpAlpha2 > 1.0f) lpAlpha2 = 1.0f;
  ...
  //  Lågpassfilter övre 
  lpFilteredSample2 = (int)(lpAlpha2 * (float)soundSampleFromADC + (1.0f - lpAlpha2) * (float)lpFilteredSample2);

  //  Bandpass 
  int lowerCentered = lpFilteredSample - 127;
  int upperCentered = lpFilteredSample2 - 127;
  int bpCentered = upperCentered - lowerCentered;
  bpFilteredSample = bpCentered + 127;

  //  Bandstopp: subtrahera bandpass från original ---
  int adcCentered = adc - 127;
  int bsCentered = adcCentered - bpCentered; // SKAPAR BANDSTOP FILTRET
  bsFilteredSample = bsCentered + 127;

  //  Skicka till PWM 
  OCR2A = (uint8_t)bsFilteredSample;
}
